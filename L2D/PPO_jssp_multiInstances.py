...
def main():
    ...
    dataLoaded = np.load('./L2D/DataGen/generatedData' + str(configs.n_j) + '_' + str(configs.n_m) + '_Seed' + str(configs.np_seed_validation) + '.npy')
    vali_data = []
    ...
    for i_update in range(configs.max_updates):
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                a_idx_envs = []
                if envs[0].done():
                    print(f"Episode steps: {step_counter}")
                    break
        ...
        # loss, v_loss = ppo.update(memories, 100, configs.graph_pool_type)
